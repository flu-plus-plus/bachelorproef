...
\newcommand{\typename}[2]{\mintinline{cpp}{#2}} % no namespaces (unqualified names)
...
\maketitle
\section*{Preface}
This document is a retrospective report for our bachelor's degree group project: a fork of the \emph{Stride} disease modeling simulator. \footfullcite{bachelorproef} We will give an overview of the features we added, and describe the hurdles and obstacles we ran into while implementing each of them.
...
We save everything used in the configuration. The files are stored as a string of characters. Other configuration variables are stored as attributes of the group containing those files. The \texttt{Atlas} and \text{Town} structures used by the visualization tool are also saved in this config map.
